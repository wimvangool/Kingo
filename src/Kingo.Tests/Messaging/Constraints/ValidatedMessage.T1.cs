...
{   
    internal sealed class ValidatedMessage<TValue> : Message
    {
        ...
        protected override void Accept(IMessageHandler handler)
        {
            handler.Handle(this);
        }

        internal ConstraintMessageValidator<ValidatedMessage<TValue>> CreateConstraintValidator(bool haltOnFirstError = false)
        {
            return new ConstraintMessageValidator<ValidatedMessage<TValue>>(haltOnFirstError);
        }
    }   
}
