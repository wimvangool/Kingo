...
namespace Kingo.MicroServices
{
    ...
    public abstract class MessageHandlerTestBase<TMessage> : ScenarioTest<IMessageHandlerTestResult>
    {
        #region [====== MessageHandlerTestResult ======]

        private sealed class MessageHandlerTestResult : TestResult<IMessageStream>, IMessageHandlerTestResult
        {
            private readonly MessageHandlerTestBase<TMessage> _scenario;

            public MessageHandlerTestResult(MessageHandlerTestBase<TMessage> scenario)
            {
                _scenario = scenario;
            }                        
            ...
            protected override Task<IMessageStream> ExecuteScenarioAsync() =>
                _scenario.Processor.HandleAsync(_scenario.When(), _scenario.CreateMessageHandler());
        ...
        /// <inheritdoc />
        protected override IMessageHandlerTestResult ThenResult() =>
            new MessageHandlerTestResult(this);

        internal override IMessageStream CreateStream() =>
            Given().Join().Append(When(), CreateMessageHandler());

        #region [====== Given / When / Then ======]          

        /// <summary>
        /// Creates and returns the message that will be handled by the processor after the <see cref="ScenarioTest.Given()"/>-sequence.
        /// </summary>
