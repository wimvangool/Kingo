...
namespace Kingo.Reflection
{
    ...
    public static class TypeExtensions
    {
        ...
        private static StringBuilder AppendFriendlyNameOf(this StringBuilder builder, Type type, bool useFullNames, bool includeGenericParameters)
        {
            ...
            {
                var friendlyName = builder.Append(typeName.RemoveTypeParameterCount());

                if (includeGenericParameters)
                {
                    friendlyName = friendlyName
                        .Append('<')
                        .AppendTypeParameters(type.GetGenericArguments(), useFullNames && !type.ContainsGenericParameters, true)
                        .Append('>');
                }
                return friendlyName;                   
            }
        ...
        {
            var indexOfQuotationMark = typeName.IndexOf("`", StringComparison.InvariantCultureIgnoreCase);
        ...
        private static StringBuilder AppendTypeParameters(this StringBuilder builder, Type[] typeParameters, bool useFullNames, bool includeGenericParameters) =>
            builder
                .AppendFriendlyNameOf(typeParameters[0], useFullNames, includeGenericParameters)
                .AppendTypeParameterTail(typeParameters, useFullNames, includeGenericParameters, 1);

        private static StringBuilder AppendTypeParameterTail(this StringBuilder builder, Type[] typeParameters, bool useFullNames, bool includeGenericParameters, int index)
        {
            ...
            return builder
                .Append(", ")
                .AppendFriendlyNameOf(typeParameters[index], useFullNames, includeGenericParameters)
                .AppendTypeParameterTail(typeParameters, useFullNames, includeGenericParameters, index + 1);
        }
