...
namespace System.ComponentModel
{
    ...
    public sealed class ManualValidationStrategy<TMessage> : IMessageValidationStrategy where TMessage : class
    {
        ...
        /// The delegate that is used to create a new <see cref="ValidationErrorTreeBuilder" /> for the specified <paramref name="message"/>.
        /// </param>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="message"/> or <paramref name="builderFactory "/> is <c>null</c>.
        /// </exception>
        public ManualValidationStrategy(TMessage message, Func<TMessage, ValidationErrorTreeBuilder> builderFactory)
        {
            if (message == null)
            {
            ...
            _message = message;
            _validator = new ManualMessageValidator<TMessage>(builderFactory);
        }
