...
{
    [TestClass]
    public sealed class PawnPromotedToQueenScenario : InMemoryScenario<PromotePawnCommand>
    {
        public PawnPromotedToQueenScenario()
        {
        ...
        {
            var message = new PromotePawnCommand(GameId, TypeOfPiece.Queen);
        ...
        private void Validate(IMemberConstraintSet<PawnPromotedEvent> validator)
        {
            ...
            validator.VerifyThat(m => m.PromotedTo).IsEqualTo(TypeOfPiece.Queen);
            validator.VerifyThat(m => m.NewState).IsEqualTo(GameState.Check);
        }
    }
}
